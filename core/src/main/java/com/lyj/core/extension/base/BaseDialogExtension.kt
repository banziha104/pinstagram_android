...
fun <VIEW_MODEL : ViewModel, VIEW_BINDING : ViewBinding> BaseDialog<VIEW_MODEL, VIEW_BINDING>.resString(
    @StringRes resId: Int
): String = requireContext().getString(resId)

fun <VIEW_MODEL : ViewModel, VIEW_BINDING : ViewBinding> BaseDialog<VIEW_MODEL, VIEW_BINDING>.resDimen(
    @DimenRes resId: Int
): Float = requireContext().resources.getDimension(resId)


fun <VIEW_MODEL : ViewModel, VIEW_BINDING : ViewBinding> BaseDialog<VIEW_MODEL, VIEW_BINDING>.getColor(
    @ColorRes resId: Int
): Int = ContextCompat.getColor(requireContext(), resId)

fun <VIEW_MODEL : ViewModel, VIEW_BINDING : ViewBinding> BaseDialog<VIEW_MODEL, VIEW_BINDING>.resDrwable(
...
val <VIEW_MODEL : ViewModel, VIEW_BINDING : ViewBinding> BaseDialog<VIEW_MODEL, VIEW_BINDING>.fromStartToStopScope
    get() = DisposableScope(EntryPoint.START at EndPoint.STOP, disposableController)

val <VIEW_MODEL : ViewModel, VIEW_BINDING : ViewBinding> BaseDialog<VIEW_MODEL, VIEW_BINDING>.fromResumeToPause
    get() = DisposableScope(EntryPoint.RESUME at EndPoint.PAUSE, disposableController)

fun <VIEW_MODEL : ViewModel, VIEW_BINDING : ViewBinding> BaseDialog<VIEW_MODEL, VIEW_BINDING>.customScope(
